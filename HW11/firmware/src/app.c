...
float IIR_X_ANG_AVG;
float IIR_Y_ANG_AVG;
float X_ANG_BUFFER[50];
float Y_ANG_BUFFER[50];
...
//            Z_L_ACC = (DATA[13] << 8) | DATA[12]; 
            ...
            float raw_x_ang = X_ANG*0.035;
            float raw_y_ang = Y_ANG*0.035;
//                X_L_ACC*0.00061, Y_L_ACC*0.00061, Z_L_ACC*0.00061, X_ANG*0.035, Y_ANG*0.035, Z_ANG*0.035
            
//IIR
            int iir_i = 0;
            int iir_dat_size = 2;
            for (iir_i = (iir_dat_size - 1); iir_i > 0; iir_i--) {
                X_ANG_BUFFER[iir_i] = X_ANG_BUFFER[iir_i - 1];
                Y_ANG_BUFFER[iir_i] = Y_ANG_BUFFER[iir_i - 1];
            }
            X_ANG_BUFFER[0] = raw_x_ang;
            Y_ANG_BUFFER[0] = raw_y_ang;
            IIR_X_ANG_AVG = iir(X_ANG_BUFFER, IIR_X_ANG_AVG);
            IIR_Y_ANG_AVG = iir(Y_ANG_BUFFER, IIR_Y_ANG_AVG);
            
//            if(IIR_X_ANG_AVG > -0.01 && IIR_X_ANG_AVG < 0.01){
//                IIR_X_ANG_AVG = 0;
//            }
            if(inc > 5){
                appData.mouseButton[0] = MOUSE_BUTTON_STATE_RELEASED;
                appData.mouseButton[1] = MOUSE_BUTTON_STATE_RELEASED;
                appData.xCoordinate = (int8_t) (IIR_X_ANG_AVG);
//                appData.yCoordinate = (int8_t) 1; 
                appData.yCoordinate = (int8_t) (IIR_Y_ANG_AVG); 
                inc = 0;
            }
            else{
