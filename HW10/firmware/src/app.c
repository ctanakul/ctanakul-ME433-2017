...
//              MAF
                int maf_i = 0;
                int maf_dat_size = 10;
                for (maf_i = (maf_dat_size - 1); maf_i > 0; maf_i--) {
                    MAF_BUFFER[maf_i] = MAF_BUFFER[maf_i - 1];
                }
                MAF_BUFFER[0] = raw_z_l_acc;
                
//              IIR
                IIR_AVG = iir(MAF_BUFFER, IIR_AVG);
                
                len = sprintf(dataOut, "%d %.2f %.2f %.2f %.2f\r\n", i, raw_z_l_acc, maf(MAF_BUFFER, maf_dat_size), IIR_AVG, fir(MAF_BUFFER, 10));
                i++;
            }
            else {
                len = 1;
