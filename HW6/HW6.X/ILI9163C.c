...
void draw_bar(unsigned short x, unsigned short y, int counter, int bar_depth, unsigned short t_color, unsigned short bg_color){

    unsigned char msg[45];
    sprintf(msg, "Hello Mars!! %d   ", counter);
    draw_string(x, y, msg, t_color, bg_color);
//    counter = counter + 50;
    
    int xs = (128/2)-50; // start column of progress bar
    int j = 0, k = 0;
    for (j = 0; j < 100; j++){
        for(k = 0; k < bar_depth; k++){
            if(counter <= 50){
                if(j < counter){
                    LCD_drawPixel(xs+j, y+20+k, bg_color);                     
                }
                else if (j <= 50) {
                    LCD_drawPixel(xs+j, y+20+k, RED);                        
                }   
                else{
                    LCD_drawPixel(xs+j, y+20+k, bg_color);                            
                }
